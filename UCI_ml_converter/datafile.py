...
class DataFile:
    ...
    def process_file(self, *files, merge: bool = False) -> pd.DataFrame:
        ...
        num_cols = sorted(list(set(range(len(df.columns) - len(self.merge_class))) - set(self.ignore_cols) - \
    ...
    def _reorder_columns(self, num_cols: list[int], df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.class_col_index == -1:
            ...
            if self.merge_class:
                class_col = self._merge_class_columns(df)
            else:
    ...
    def _merge_class_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
